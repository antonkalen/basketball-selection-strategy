make_table_2 <- function(model, model_moderator, width) {
  
  model_table_1 <- make_table_2_part(model = model, width = width)
  model_table_2 <- make_table_2_part(model = model_moderator, width = width)
  ...
  full_table <- model_table_1 %>% 
    ...
    dplyr::arrange(
      ...
      gender,
      match(variable, c("intercept", "nr_youth_m", "nr_youth_cv", "players_lic", "ranking_points_senior"))
    )

  full_table
}

make_table_2_part <- function(model, width) {
  
  # Get draws
  draws_summary <- model %>% 
    tidybayes::gather_draws(`b.*`, `sigma`, regex = TRUE) %>% 
    ggdist::mean_hdi() %>% 
  ...
  draws_cleaned <- draws_joined %>% 
    dplyr::mutate(
      ...
      variable = dplyr::case_when(
        ...
        stringr::str_detect(.variable, "sigma") ~ "sigma",
        TRUE ~ "intercept"
      )
    )
